...
def rk4_step(h, u, v):
    h1, u1, v1 = rhs(h, u, v)
    h2, u2, v2 = rhs(h + (dt/2)*h1, u + (dt/2)*u1, v + (dt/2)*v1)
    h3, u3, v3 = rhs(h + (dt/2)*h2, u + (dt/2)*u2, v + (dt/2)*v2)
    h4, u4, v4 = rhs(h + dt*h3,     u + dt*u3,     v + dt*v3)
...
# ----------------------------------------------------------------
# RK2 run with live plots (original simulation)
# ----------------------------------------------------------------
...
def update_rk2(h, u, v, steps=60000):
    for n in range(steps):
        ...
        if n % 500 == 0:
            ...
            plt.contourf(X/1e3, Y/1e3, zeta, levels=50, cmap='seismic')
...
def update_rk2_anim(h, u, v, steps=60000):
    ...
    for n in range(steps):
        ...
        h, u, v = boundary_conditions(h, u, v)
        if n % 100 == 0:
            zeta_frames.append(ddx(v) - ddy(u))
...
print(f"WIth nu = 1000, Re = 8 * 20000/1000 = {8 * 20000/1000}")
...
print(f"Theoretical Strouhal:St  = 0.200")
print(f"Difference:{abs(St_empirical - 0.2)/0.2*100:.1f}%")
print(f"WIth nu = 5000, Re = 8 * 20000/1000 = {8 * 20000/5000}")
...
def update_rk4_anim_island(h, u, v, steps=60000):
    zeta_frames2 = []
    for n in range(steps):
        h, u, v = rk4_step(h, u, v)
        h, u, v = boundary_conditions(h, u, v)
        if n % 100 == 0:
            zeta_frames2.append(ddx(v) - ddy(u))
...
anim2.save('vorticity_animation_island.mp4', writer='ffmpeg', fps=35, dpi=120)
print("saved vorticity_animation_island.mp4")
plt.show()
